...
vecteur vecteur2D::Addition(vecteur2D &vec)
{
...
vecteur vecteur2D::Produit_de_Composante(vecteur2D &vec)
{
	return vecteur2D(vec.x * x, vec.y * y);
}
...
vecteur vecteur2D::Multiplication_Scalaire(float k)
{
	return vecteur2D(x * k, y * k);
}

vecteur vecteur2D::addForce(vecteur2D &force)
{
	return vecteur2D(x + force.x, y * force.y);
}

vecteur vecteur2D::clearAccum()
{
